...
from plurals.helpers import load_yaml, format_previous_responses, format_previous_responses_debate
...
class Moderator(Agent):
    ...
    def moderate_responses(self, responses: List[str], original_task: str) -> str:
        ...
        """
        combined_responses_str = format_previous_responses_debate(responses, who="moderator")
...
class Debate(Chain):
    def __init__(self, agents: List[Agent], task_description: Optional[str] = None, shuffle: bool = False, cycles: int = 1, last_n: int = 1, combination_instructions: Optional[str] = "default", moderator: Optional[Moderator] = None):
        if len(agents) != 2:
    ...
    def process(self):
        ...
        for _ in range(self.cycles):
            for agent in self.agents:
                previous_responses_slice = previous_responses[-self.last_n:]
                previous_responses_str = format_previous_responses_debate(previous_responses_slice, who="agent")
                agent.combination_instructions = self.combination_instructions
                response = agent.process_task(previous_responses_str)
                previous_responses.append(response)
                self.responses.append(response)
